...
import { sendPassword, changePassword } from "services/Auth/SignUp";
...
export function* handleChangePassword(action: any) {
  try {
    const response = call(() => changePassword(action.payload));
    const { data } = yield response;
    yield put(setMessage({message: data.message, isSuccessful: data.isSuccessful}));
    sleep(500);
		yield put(setMessage({ message: "", isSuccessful: false }));
